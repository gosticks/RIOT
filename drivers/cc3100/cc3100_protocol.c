...
{
    spi_acquire(1, SPI_CS_UNDEF, SPI_SUB_MODE_0, SPI_RATE_20M);
...
int cc3100_read_cmd_header(cc3100_nwp_resp_header_t *buf)
{
    ...
    cc3100_send_to_nwp(&CPU_TO_NWP_CNYS_PATTERN.Short, sizeof(uint32_t));
    uint32_t SyncCnt = 0;
    ...
    if (SyncCnt > 0) {
        *(uint32_t *)buf = *(uint32_t *)&buf[4];
        cc3100_read_from_nwp(&buf[4 - SyncCnt], (uint16_t)SyncCnt);
    } else {
        cc3100_read_from_nwp(buf, 4);
    }
    ...
    cc3100_read_from_nwp(buf, (uint8_t)((SyncCnt > 0) ? (4 - SyncCnt) : 0));
    return 0;
}

void send_header(cc3100_nwp_header_t *header)
{
    cc3100_send_to_nwp(header, sizeof(cc3100_nwp_header_t));
}
...
{
    cc3100_send_to_nwp(&CPU_TO_NWP_SYNC_PATTERN.Short, sizeof(uint32_t));
}

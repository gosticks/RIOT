...
typedef struct DriverMessage {
	...
	uint8_t cmdDescLen; // length of the command description (not protocol aligned)
	uint8_t *payload;
	uint16_t payloadLen;
...
#define DEVICE_INFO_RX_LEN                                \
	sizeof(_DeviceSetGet_t) + sizeof(SlVersionFull) + \
...
uint8_t sendCommand(DriverMessage *msg, DriverResponse *res)
{
	...
	uint16_t	   cmdDescLen = alignDataLen(msg->payloadHeaderLen +
					     msg->payloadLen + msg->cmdDescLen);
	_SlGenericHeader_t header     = { .Opcode = msg->Opcode,
					  .Len    = cmdDescLen };
...
int16_t connect(WifiProfileConfig *conf)
{
	...
	if (NULL != conf->key) {
		memcpy(PASSWORD_STRING(&Msg), conf->key,
...
int16_t profileAdd(WifiProfileConfig *conf)
{
	...
	if (NULL != conf->ssid) {
		/* copy SSID */
		memcpy(PROFILE_SSID_STRING(&Msg), conf->ssid,
		       conf->common.SsidLen);
	}
...
int16_t disconnectFromWifi(void)
{
	...
	DriverMessage msg = { .Opcode = SL_OPCODE_WLAN_WLANDISCONNECTCOMMAND,
			      .RespOpcode =
				      SL_OPCODE_WLAN_WLANDISCONNECTRESPONSE,
			      .cmdDesc    = NULL,
			      .cmdDescLen = 0,
			      .payloadLen = 0 };
			     ...
			     int16_t options)
{
	...
	for (int i = 0; i < chunksCount; i++) {
		...
		msg.payloadLen = len - bufOffset > packetLen ? packetLen :
							       len - bufOffset;
...
int16_t setWlanFilter(uint8_t filterOptions, uint8_t *inBuf, uint16_t bufLen)
{
	...
	DriverMessage msg = { .Opcode = SL_OPCODE_WLAN_WLANRXFILTERSETCOMMAND,
			      .RespOpcode =
				      SL_OPCODE_WLAN_WLANRXFILTERSETRESPONSE,
			      .cmdDesc    = (uint8_t *)&data,
